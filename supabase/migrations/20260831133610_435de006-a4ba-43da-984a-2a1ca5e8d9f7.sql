DROP FUNCTION IF EXISTS public.finalize_order_payment(uuid, text, text, text, text, uuid, text);
...
CREATE OR REPLACE FUNCTION public.finalize_order_payment(
  ...
  p_online_payment_id uuid DEFAULT NULL,
...
END;
$$;
REVOKE ALL ON FUNCTION public.finalize_order_payment(uuid, text, text, text, uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.finalize_order_payment(uuid, text, text, text, uuid, uuid, text) TO anon, authenticated, service_role;