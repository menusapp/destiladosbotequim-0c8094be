# Destilado Botequim — ERP e cardápio digital

Sistema de gestão para restaurante: cardápio digital com QR Code por mesa,
comanda, PDV, delivery (próprio, iFood e Delivery Direto), fiscal (NFC-e),
estoque, fidelidade, marketing por WhatsApp e painel de CEO.

Stack: React 18 + Vite + TypeScript + Tailwind/shadcn no front, Supabase
(PostgreSQL + Edge Functions em Deno) no back.
...
## Migrando para o seu próprio Supabase

Este repositório não depende mais do Lovable. Para subir tudo num projeto
Supabase seu, o passo a passo completo está em **[MIGRACAO.md](./MIGRACAO.md)**.

Resumo:

```sh
cp .env.example .env               # chaves públicas do frontend
cp .env.supabase.example .env.supabase   # secrets das edge functions
...
| Script | Para quê |
|---|---|
| `scripts/configurar-supabase.sh` | Faz tudo: link, migrations, secrets, deploy e cron |
| `scripts/deploy-functions.sh` | Só o deploy das edge functions |
| `scripts/verificar-migracao.sh` | Confere se o projeto novo ficou correto |

## Estrutura

```
src/
  components/admin/    painel do restaurante (PDV, pedidos, cardápio, estoque…)
  components/ceo/      painel do CEO
  components/menu/     cardápio público e checkout
  components/kiosk/    modo totem
  integrations/        cliente Supabase e tipos gerados
supabase/
  functions/           45 edge functions (Deno)
  functions/_shared/   código compartilhado entre functions
  migrations/          217 migrations
scripts/               automação da migração e do deploy
```

## Documentação

- **[MIGRACAO.md](./MIGRACAO.md)** — sair do Lovable, rodar no seu Supabase
- [SECURITY_HARDENING_RUNBOOK.md](./SECURITY_HARDENING_RUNBOOK.md) — modelo de sessão e RLS
- [README_DESIGN_SYSTEM.md](./README_DESIGN_SYSTEM.md) — tokens e componentes
- `LOVABLE_DEPLOY_PROMPT.md` — histórico; as migrations que ele descrevia já estão aplicadas

---

<details>
<summary>Descrição original do produto (prompts que originaram o sistema)</summary>
